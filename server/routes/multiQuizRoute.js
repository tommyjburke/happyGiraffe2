...
router.post('/add-multi-quiz', authMiddleware, async (req, res) => {
   ...
   try {
      ...
      res.send({
         message: 'Multiple Choice Quiz added good good',
         success: true,
...
//       res.send({
//          message: 'Multi Quizzes retrieved good good',
//          data: multis,
//          success: true,
//       })
//    } catch (error) {
...
// get all multiple choice by Teacher ID
router.post('/get-all-multis-by-teacher-id', authMiddleware, async (req, res) => {
   try {
      const multis = await MultiQuiz.find({ teacherId: req.body.userId })
      res.send({
         message: 'Multi Quizzes retrieved good good',
         data: multis,
         success: true,
...
router.post('/get-multi-by-id', authMiddleware, async (req, res) => {
   try {
      ...
      res.send({
         message: 'MultiQuiz retrieved good good',
         data: multi,
         success: true,
      })
   } catch (error) {
...
router.post('/delete-multi-by-id', authMiddleware, async (req, res) => {
   try {
      ...
      res.send({
         message: 'MultiQuiz and related questions deleted good good',
         success: true,
      })
   } catch (error) {
      res.status(500).send({
...
router.post('/add-question-to-multi', authMiddleware, async (req, res) => {
   try {
      ...
      await multi.save()
      res.send({
         message: 'Question added good good',
         success: true,
      })
   } catch (error) {
      res.status(500).send({
...
router.post('/edit-question-by-id', authMiddleware, async (req, res) => {
   try {
      ...
      res.send({
         message: 'Question edited good good',
         success: true,
      })
   } catch (error) {
      res.status(500).send({
...
router.post('/delete-question-in-multi', authMiddleware, async (req, res) => {
   try {
      ...
      await multi.save()
      res.send({
         message: 'Question deleted good good',
         success: true,
      })
   } catch (error) {}
})
